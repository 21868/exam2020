...
public class Text {
        ...
        public static void save_user_searches(ArrayList<String> searched_cities, String traveller_name){
            // Grafw kainourgio arxeio me ta searches tou kathe xristi
            String line_2_write = traveller_name ;
            try {
                FileWriter myWriter = new FileWriter("searches.txt", true);
                for(String city: searched_cities){
                    line_2_write += " " + city;
                }
                myWriter.write(line_2_write + "\n");
                myWriter.close();
            } catch (IOException e) {
                System.out.println("An error occurred.");
                e.printStackTrace();
        ...
        public static ArrayList<String> read_searches(){
            //Diavazw ta searches
            ArrayList<String> searches = new ArrayList<>();
            try {
                File searchesFile = new File("searches.txt");
                Scanner myReader = new Scanner(searchesFile);

                while (myReader.hasNextLine()) {
                    String[] line_elements = myReader.nextLine().split(" ");

                    // Pairnw apo to 2 kai meta giati ta prwta einai to onoma
                    searches.addAll(Arrays.asList(line_elements).subList(2, line_elements.length));
                }
        ...
        static int countWords(String str) { // returns number of words in str
            int OUT = 0;
            int IN = 1;
